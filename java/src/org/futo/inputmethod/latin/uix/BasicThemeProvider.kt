...
class BasicThemeProvider(val context: Context, val colorScheme: KeyboardColorScheme) :
    ...
    private fun makeVisualStyle(background: Int, foreground: Int, highlight: Int, foregroundPressed: Int, roundedness: Dp): VisualStyleDescriptor {
        ...
        val rippleColor = Color(highlight).copy(alpha = 0.4f).toArgb()
        val bgPressed = if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
             android.graphics.drawable.RippleDrawable(
                 android.content.res.ColorStateList.valueOf(rippleColor),
                 coloredRoundedRectangle(background, dp(roundedness)),
                 coloredRoundedRectangle(android.graphics.Color.WHITE, dp(roundedness))
             )
        } else {
            coloredRoundedRectangle(Color(highlight).compositeOver(Color(background)).toArgb(), dp(roundedness))
        }
